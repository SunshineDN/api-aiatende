...
export default class WebCalendarController {
  ...
  static async choice(req, res) {
    try {
      ...
      const response = await calendar.getChoiceDate(data, turno, dentista);
  ...
  static async register(req, res) {
    try {
      const { dentista, data, horario, query } = req.body;
      const calendar = new WebCalendarServices(query);
      const response = await calendar.insertEvent(dentista, data, horario);

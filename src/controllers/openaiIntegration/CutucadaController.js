...
export default class CutucadaController {
  ...
  static async gerar_perguntas_historico(req, res) {
    try {
      const { lead_id } = req.body;
      const cutucadaServices = new CutucadaServices(lead_id);
      const response = await cutucadaServices.gerar_perguntas_historico();
      return res.status(response.code).send(response);
    } catch (error) {
      styled.error(`[CutucadaController.gerar_perguntas_historico] Erro ao enviar mensagem para o prompt: ${error?.message}`);
      console.error(error);
      return res.status(500).send({ message: 'Erro ao enviar mensagem para o prompt', error: error?.message });
    }
  }

...
const GetAccessToken = async (payload) => {
  ...
  try {
    const { data: { access_token } } = await axios.post('https://oauth.aiatende.com.br/auth/access_token', {
      account_id
    });
    ...
    try {
      ...
      const { data: { access_token } } = await axios.post('https://oauth.aiatende.com.br/auth/access_token', req);
